...
public class MenuView {
	...
	public static void menu() {
		while(true) {
			...
			System.out.println(ss.getSet()); //비어있는 세션이 나옴
			...
			case 1 :
				...
				break;
			case 2 :
				MenuView.login();// 로그인. 로그인을 하지 않으면 아무것도 못함
				break;
	...
	public static void logout(String userId) {
		...
		// 새로운 객체를 만들어서 제거하는 이유->다른 객체이지만 hashcode & equals 함수 조작해서 같은 아이디면 삭제 되도록 설계
		// 아이디를 가지고 판단하기 때문에 다른 객체여도 아이디가 같으면 같은 객체라고 생각해서 세션에서 제거됨
		// 주소값으로 판단하지 않기 때문에 새로운 객체를 만들어서 판단해도 된다
		SessionSet ss = SessionSet.getInstance();
		ss.remove(session);	
	}
    ...
    public static void printInputOrder(String userId) {
    	System.out.print("주문상품번호 : ");
		 ...
		 OrderLine orderLine = new OrderLine(0, 0, goodsId, 0, qty, 0); //상세
